...
/*   Updated: 2023/04/09 14:36:03 by mben-sal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while(i < jeux->longeur_map)
	{
		...
		while(j < jeux->lnmap)
		{
			if(jeux->map2[i][j] == 'C')
			{
				...
				{
					ft_message_erreur_bonus("1 - erreur: le joueur ne trouve pas aucun chemin\n");
					exit(1);
				}
			}
			j++;
...
void ft_validation_chemin_bonus(t_game_bonus *jeux)
{
	...
	{
		ft_message_erreur_bonus("2 - erreur : le joueur ne trouve pas aucun chemin\n");
		exit(1);
	}
